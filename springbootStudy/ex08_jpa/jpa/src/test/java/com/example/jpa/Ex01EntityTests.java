...
class Ex01EntityTests {
  ...
  void testEntityMapping() {
    ...
    Product product = new Product("P002","다른상품",1500000,"가볍고 높은 성능의 가성비 노트북");

    //엔티티 매니저가 엔티티를 관리함
    //세부 핵심 특징 : 관리되는 엔티티는 반드시 ID를 가지고 있어야 한다. AUTO_INCREMENT 전략을 사용하는 경우
    // persist() 호출 즉시 INSERT 쿼리를 DB로 날려 ID를 받아온 뒤 영속성 컨텍스ㅌ에 저장한다.
    em.persist(product);

    //ID 확인
    assertNotNull(product.getId()); // ID 가 NOT NULL이면 통과
  ...
  void entityTest(){
    User user = new User("홍길동" , "gildonge", Role.USER);
    em.persist(user);
